...
namespace OmmoBackend.Middlewares
{
    public class RolePermissionMiddleware
    {
        ...
        private static readonly List<(string Path, string Module, string Component, int AccessLevel)> ApiPermissions = new()
        {
            ...
            ("/api/role/get-roles", "Setting", "User", 1),
            ...
            ("/api/loadboard/get-loads", "Load Board", "Calls", 1),
            ("/api/integration/send-integration-request", "Setting", "Integration", 2),
            ("/api/call/get-called-loads", "Load Board", "Calls", 1),
            ("/api/call/takeover", "Load Board", "Calls", 1),
            ("/api/aiagent/outbound", "Load Board", "Calls", 1),
            ("/api/integration/toggle-integration-status", "Setting", "Integration", 1),
            ("/api/aiagent/settings", "Setting", "AI Agents", 1),
            ("/api/aiagent/update-settings", "Setting", "AI Agents", 2),
            ("/api/Transcript/gettranscript", "Load Board", "Calls", 1),
            ("/api/call/get-calls", "Load Board", "Calls", 1),
        };
        ...
        public async Task InvokeAsync(HttpContext context, IRoleModuleService roleModuleService)
        {
            try
            {
                ...
                // Authentication already handled by UseAuthentication()
                ...
                var companyIdClaim = context.User.Claims.FirstOrDefault(c => c.Type == "Company_ID");
                var roleIdClaim = context.User.Claims.FirstOrDefault(c => c.Type == "Role_ID");
                ...
                // Skip permission checks for endpoints
                ...
                if (!apiPermissions.Any())
                {
                    ...
                    throw new NotFoundException("API path is not recognized.");
                }
            ...
            {
                _logger.LogError(ex, "Error occurred in RolePermissionMiddleware");

                //context.Response.StatusCode = StatusCodes.Status500InternalServerError;
                //await context.Response.WriteAsync("An error occurred during authentication.");

                throw; // Let the global exception handler middleware handle the response
            }
        }
